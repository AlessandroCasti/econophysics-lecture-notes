...
ax.set_xlim(min(-6, z.min()-0.5), max(6, z.max()+0.5))
...
ax.plot(lags, 0.32 * lags**-0.3, color=RED, lw=1.1,
        label=r"guide: $\propto \tau^{-0.3}$")
ax.axhspan(-noise, noise, color=GRAY, alpha=0.18, lw=0)
ax.set_xscale("log")
ax.set_xlabel(r"time lag $\tau$ (days)")
ax.set_ylabel("autocorrelation")
ax.set_ylim(min(-0.1, acf_r.min()-0.025, acf_abs.min()-0.025), max(0.45, acf_r.max()+0.025, acf_abs.max()+0.025))
ax.legend(frameon=False)
