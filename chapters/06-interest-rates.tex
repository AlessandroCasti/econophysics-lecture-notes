...
\subsection{Constant interest rate}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item The EMH holds, so the dynamics is It\^o (Markov), without memory.
...
Concretely, consider a trader with capital $\Pi(t)$ who
(a) buys a bond with maturity $T_1 = 1$ year, $B_1 = B(r,t;T_1)$, so that the wealth is $W = \Pi(t) + B_1$, exposed to the fluctuations of $r$ through $B_1$;
(b) sells a second bond $B_2 = B_2(r,t;T_2)$ with $T_2 = 3$ years --- same underlying rate, hence stochastically correlated with the first;
(c) so that the total wealth is $W(t) = B_2(t) = \Pi(t) + B_1(t)$.
The portfolio is therefore
\begin{equation}
\Pi(t) = B_2(t) - B_1(t):
\end{equation}
long one maturity, short the other (the trader is simultaneously writer and holder).

Differentiating, and using for each bond its drift and volatility as defined above,
\begin{equation}
d\Pi(t) = dB_2(t) - dB_1(t) = \bigl\{B_2\,\mu(t,T_2) - B_1\,\mu(t,T_1)\bigr\}dt - \bigl\{B_2\,\sigma(t,T_2) - B_1\,\sigma(t,T_1)\bigr\}dW_t.
\end{equation}
By the no-arbitrage principle we eliminate the market risk, setting the coefficient of $dW_t$ to zero:
\begin{equation}
B_2\,\sigma(t,T_2) - B_1\,\sigma(t,T_1) = 0.
\end{equation}
Together with the budget constraint $\Pi = B_2 - B_1$, this fixes the sizes of the two positions:
\begin{align}
(a)\quad & (\Pi + B_1)\,\sigma(t,T_2) - B_1\,\sigma(t,T_1) = 0 &&\Longrightarrow\quad B_1 = \frac{\Pi\,\sigma(t,T_2)}{\sigma(t,T_1) - \sigma(t,T_2)}, \\[4pt]
(b)\quad & B_2\,\sigma(t,T_2) - (B_2-\Pi)\,\sigma(t,T_1) = 0 &&\Longrightarrow\quad B_2 = \frac{\Pi\,\sigma(t,T_1)}{\sigma(t,T_1) - \sigma(t,T_2)}.
\end{align}
Substituting these optimal values back into $d\Pi$, the surviving deterministic part is
\begin{equation}
d\Pi(t) = \Pi\,\frac{\mu(t,T_1)\,\sigma(t,T_2) - \mu(t,T_2)\,\sigma(t,T_1)}{\sigma(t,T_1) - \sigma(t,T_2)}\, dt.
\end{equation}

The portfolio is now riskless, and the no-free-lunch argument of Chapter~5 applies verbatim: it must earn the risk-free rate,
\begin{equation}
\frac{d\Pi(t)}{\Pi(t)} = r\,dt = \frac{dB(t)}{B(t)},
\end{equation}
which, after clearing denominators, becomes
\begin{equation}
\sigma(t,T_1)\bigl[\mu(t,T_2) - r\bigr] = \sigma(t,T_2)\bigl[\mu(t,T_1) - r\bigr].
\end{equation}
...
\subsection{The market price of risk}
...
Third, regarding $q(r,t) = \frac{\mu - r}{\sigma}$: when $\mu$ rises, $\sigma$ typically rises with it, so the ratio is roughly constant, $q\approx\text{const}$, a useful approximation in practice.
Finally, the Feynman--Kac formula of Chapter~5 offers an alternative representation of the solution,
\begin{equation}
B(r,t;T) = \hat{B}(T)\,\mathbb{E}\!\Bigl[\exp\Bigl\{-\int_t^T r(s)\,ds\Bigr\}\Bigr],
\end{equation}
...
Here $\theta$ is the \emph{long-run mean} toward which the rate is pulled, and we note already that $\theta$ is \emph{not} simply the historical mean of the observed time series.\footnote{The parameter $\theta$ is determined from the market, not from historical data alone. See Section~\ref{sec:vasicek-calibration}.}
...
\end{equation}
the mean-reverting drift has been absorbed, exactly as in Doob's transform, and only an inhomogeneous source survives.
...
\end{equation}
The anatomy of this solution is worth reading term by term: the initial rate decays away exponentially, the long-run mean $\theta$ fades in to replace it, and the noise accumulates with exponentially fading memory.

The moments follow at sight.
The It\^o integral averages to zero, so
\begin{equation}
\mathbb{E}[r(T)] = \theta\bigl(1 - e^{-\alpha(T-t)}\bigr) + r(t)\, e^{-\alpha(T-t)} \;\xrightarrow{T-t \gg \tau}\; \theta,
\end{equation}
and the mean converges to $\theta$ for long times --- the long-run economic average.
By the It\^o isometry,
\begin{equation}
\mathrm{Var}[r(T)] = \sigma^2\, e^{-2\alpha T}\int_t^T e^{2\alpha s}\, ds = \frac{\sigma^2}{2\alpha}\bigl(1 - e^{-2\alpha(T-t)}\bigr) \;\xrightarrow{T-t \gg \tau}\; \frac{\sigma^2}{2\alpha},
\end{equation}
so the fluctuations saturate rather than growing without bound.
The process being Gaussian at all times,
\begin{equation}
\mathrm{PDF}\{r(t)\} \sim \mathcal{N}(r(t)) \;\xrightarrow{(T-t)\gg\tau}\; \mathcal{N}\!\Bigl(\theta,\, \frac{\sigma^2}{2\alpha}\Bigr),
\end{equation}
while for short times the mean expands as $\mathbb{E}[r(t)] \approx r + (\frac{\theta}{\sigma} - 1)\,\alpha(T-t) + \ldots$
...
In simulations one generates infinitely many trajectories and takes the ensemble average, recovering exactly this relaxation.
...
The extreme case is Switzerland: the Swiss National Bank held its policy rate at $-0.75\%$ from 2015 to 2022 --- the deepest negative rate ever set by a central bank --- and for stretches of that period the \emph{entire} Swiss government yield curve, out to 50-year maturities, traded below zero.}
...
Still, negativity is not the standard, and an alternative exists: the CIR model \cite{CIR1985} (Cox--Ingersoll--Ross), the non-Gaussian variant defined in the notes by
\begin{equation}\label{eq:cir-sde}
d\sqrt{r} = -\gamma\sqrt{r}\, dt + \delta\, dW_t,
\end{equation}
% CLAUDE NOTE: The notes write dr̃ = -γ√r dt + δ dW_t with PDF{√r(t)} ~ N(√r(t)), i.e. an
% OU process for x=√r. The standard CIR model is dr = κ(θ-r)dt + σ√r dW_t, with the √r in
% the diffusion term. The derivation below (from the notes) shows the two are related by Itô.
where the Gaussian variable is $x := \sqrt{r}$, so that $\mathrm{PDF}\{\sqrt{r(t)}\} \sim \mathcal{N}(\sqrt{r(t)})$ and $f(x)=x^2=r$.
What SDE does $r$ itself obey?
By It\^o's formula applied to $f(x)=x^2$ along $dx = -\gamma x\, dt + \delta\, dW_t$,
\begin{equation}
df(x) = dx^2 = \bigl\{-\gamma x \cdot 2x + \tfrac{1}{2}\cdot 2\cdot\delta^2\bigr\}dt + 2\delta x\, dW_t = \bigl(\delta^2 - 2\gamma x^2\bigr)dt + 2\delta x\, dW_t,
\end{equation}
which, rewritten in terms of $r=x^2$, takes the form
\begin{equation}
dr(t) = \alpha\bigl[\theta - r(t)\bigr]dt + \sigma\sqrt{r(t)}\, dW_t,
\end{equation}
with $\alpha = 2\gamma$ and $\theta = \delta^2/(2\gamma)$: a mean-reverting drift again, but with a diffusion $\propto\sqrt{r}$ that switches off as $r\to0$ --- this is what keeps the rate positive.
The process is of the Feller type.\footnote{For simulations one exploits the construction directly: $r$ is a sum of squared Gaussians, so $\mathrm{PDF}\{r(t)\} \sim \chi^2(r(t))$, the chi-squared distribution, whose support has a hard floor at zero.}
% CLAUDE NOTE: the notes write "processo di Füller"; the name is William Feller, whose 1951
% analysis of the square-root diffusion is why mathematicians call CIR the Feller process.
% The related "Feller condition" 2κθ ≥ σ² guarantees that the rate never touches zero.

\begin{figure}[h!]
\centering
\begin{tikzpicture}
    \draw[->] (0,0) -- (4,0) node[right] {$r(t)$};
    \draw[->] (0,0) -- (0,2.5) node[above] {$\chi^2(r(t))$};
    \draw[thick, blue, domain=0.05:3.8, samples=80]
        plot (\x, {2*\x*exp(-\x)});
    \node[font=\footnotesize] at (2.5, 1.8) {$\chi^2$ distribution};
\end{tikzpicture}
\caption{The stationary $\chi^2$-type distribution of the CIR rate: unlike the Vasi\v{c}ek Gaussian, its support is bounded below at zero, so negative rates never occur.}
\label{fig:cir-chi2}
\end{figure}
...
Substituting $f = \alpha(\theta-r)$ and $S=\sigma$, the drift correction combines with the market price of risk into
\begin{equation}
f + S\,q = \alpha(\theta^* - r),
\qquad
...
\begin{equation}
\ln A(t,T) = \frac{1}{2}\int_t^T \bigl[\theta^*(s,T)\,\partial_s B_0^2 - \sigma^2\,B_0^2(s,T)\bigr]ds,
\end{equation}
...
\end{equation}
There is no direct inversion from the historical time series of rates alone: in Bj\"ork's phrase, the model must be ``\emph{tirato fuori dal mercato}'' --- extracted from the market, not only from history.\cite{Bjork2009}
% CLAUDE NOTE: "tirato fuori dal mercato" = extracted/calibrated from market data.
...
\end{equation}
and solves for $\theta^*$.
Figure~\ref{fig:yield-curve} sketches the typical situation: an upward-sloping empirical curve, with the fitted model interpolating through it.

\begin{figure}[h!]
\centering
\begin{tikzpicture}
    \draw[->] (0,0) -- (6,0) node[right] {Maturity (years)};
    \draw[->] (0,0) -- (0,3) node[above] {$R(t,T)$};
    \draw[thick, blue, dashed, domain=0.3:5.5, samples=50]
        plot (\x, {2.5 - 1.5*exp(-0.5*\x)});
    \foreach \x in {0.5, 1, 1.5, 2, 2.5, 3, 4, 5} {
        \fill[blue] (\x, {2.5 - 1.5*exp(-0.5*\x) + 0.1*rand}) circle (2pt);
    }
    \node[font=\footnotesize] at (0.3,0.3) {1\%};
    \node[font=\footnotesize] at (0.3,2.2) {5\%};
\end{tikzpicture}
\caption{Calibration on the yield curve: empirical yields at the quoted maturities (dots) and the fitted Vasi\v{c}ek curve $R_{\text{the}}$ (dashed). The fit of the curve determines the risk-adjusted mean $\theta^*$.}
