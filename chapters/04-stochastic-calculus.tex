...
The \emph{white noise} $\{\xi(t)\}_{t\ge0}$ is a continuous-time stochastic process satisfying three properties:
...
\section{It\^o integrals}\cite{Ito1944}

Before stating It\^o's formula we collect the small toolbox of results about It\^o integrals that makes every later computation a few lines long.
...
We now need the statistics of the \emph{squared} increment $\Delta W^2 = [W(t)-W(s)]^2$, the so-called quadratic variation.
...
\end{equation}
Keep these two numbers in mind: the squared increment has mean of order $\Delta t$ but fluctuations of order $\Delta t^2$.
...
\end{equation}
This is no accident.
Call $G(t)$ \emph{non-anticipating} if, for every $t'<t$, it is statistically independent of the future increments $W(s)-W(t)$ for $s>t$ --- in financial language, if it does not use inside information about the future.
Then
\begin{equation}
...
\end{equation}
The remarkable statement is the boxed-worthy identity on the right: \emph{the squared Wiener differential is not random at all} --- it equals $dt$, deterministically.
...
It occupies a special place in the theory: by \emph{Doob's theorem}\cite{Doob1942,UhlenbeckOrnstein1930} it is the unique process that is simultaneously Markov, Gaussian, and strictly stationary (in the strong sense, $P_t(x,t)=P_t(x)$).
...
To solve it we use the substitution $f(x,t)=x\,e^{\kappa t}$ (Doob's transform), designed precisely to absorb the restoring drift.
By It\^o's formula,
\begin{equation}
...
\end{equation}
so the process reaches a genuinely \emph{stationary} regime, with no residual time dependence: the injection of noise ($D$) and the dissipation of the restoring force ($\kappa$) balance.
...
\end{equation}
In the original physical setting, where $x$ is the velocity of a Brownian particle, the stationary law $P^{\mathrm{O.U.}}(v) = P^{\mathrm{M.B.}}(v)$ is nothing but the Maxwell--Boltzmann distribution (Appendix~\ref{app:stationary}): the OU process describes, in real time, how an out-of-equilibrium velocity distribution relaxes to thermal equilibrium, and matching the two stationary variances yields the Einstein fluctuation--dissipation relation, Equation~\eqref{eq:einstein-relation}.
...
\begin{tikzpicture}[scale=1.0]
  ...
  \draw[dashed] (0,0.5) -- (7.0,0.5) node[right]{\small $\langle x\rangle\to 0$ level};
